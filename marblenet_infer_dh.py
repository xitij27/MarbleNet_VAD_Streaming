...
class MarbleNet(Vad):
    ...
    def infer_file(self, file_source, rttm_source, sampling_rate = 16000):
        chunk_ms = 500
        overlap = 0.75  # THIS IS OVERLAP NOT STRIDE
        chunk_sample = int(sampling_rate * chunk_ms / 1000)
        step_sample = int(sampling_rate * chunk_ms * (1 - overlap) / 1000)

        global chunk_sec, stride, ctr  # Declare as global variables
        chunk_sec = str(chunk_ms/1000)
        ...
        for st_sample in range(0, len(audio), step_sample):
            ...
            # chunk_size = audio_chunk.size()[-1]

            # if chunk_size < chunk_sample:
            #     # Pad audio_chunk to match chunk_sample size
            #     pad_size = chunk_sample - chunk_size
            #     audio_chunk = torch.nn.functional.pad(audio_chunk, (0, pad_size), mode='constant', value=0)
            # elif chunk_size > chunk_sample:
            #     # Trim audio_chunk to match chunk_sample size
            #     audio_chunk = audio_chunk[:chunk_sample]
...
if __name__ == "__main__":
    ...
    vad = MarbleNet()
    samples_dir = "/home/users/ntu/kshitij0/scratch/datasets/third_dihard_challenge_eval/data/flac"
    rttm_dir = "/home/users/ntu/kshitij0/scratch/datasets/third_dihard_challenge_eval/data/rttm"
    ...
    if len(sorted_audio_files) == len(sorted_rttm_files):
        ...
        print('======================== Results ========================')
        print(f'Chunk Size = {chunk_sec}s, stride = {stride}s')
